...
pub const SYSTEMD_SERVICE_FILE: &str = "/etc/systemd/system/pr2-agent.service";
...
fn install_crontab(executable: &PathBuf) -> Result<(), crate::Error> {
  ...
  let mut crontab_file = cfg::get_agent_directory()?;
