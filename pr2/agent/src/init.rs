use crate::{cfg, install, Error};
use proto::crypto;
use rand::RngCore;
use std::{convert::TryInto, fs};
use x25519_dalek::{x25519, X25519_BASEPOINT_BYTES};

pub fn init_and_install() -> Result<cfg::Config, Error> {
...
pub fn register() -> Result<cfg::Config, Error> {
  ...
  let client_public_key_bytes = base64::decode(cfg::CLIENT_IDENTITY_PUBLIC_KEY)?;
...
pub fn save_agent_config(conf: &cfg::Config) -> Result<(), Error> {
  let agent_config_file = cfg::get_agent_config_file_path()?;

  let serialized_conf: cfg::SerializedConfig = conf.into();
...
pub fn get_saved_agent_config() -> Result<Option<cfg::Config>, Error> {
  let agent_id_file = cfg::get_agent_config_file_path()?;

  if agent_id_file.exists() {
    let agent_file_content = fs::read(agent_id_file)?;

    let serialized_conf: cfg::SerializedConfig = serde_json::from_slice(&agent_file_content)?;
